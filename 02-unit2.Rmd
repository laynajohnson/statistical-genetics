...
In our plot, we can see that our first PC has the highest proportion of variation compared to the rest. PC1 explains around 15% of the variance and then there is a drop for the rest of the PCs. We mentioned before that our method will have the first PCs explain the most variance, and this holds true for the PCA we ran on our simulated genetic data as well. 
...
We see with our plot that our SNP4 does indeed have the smallest p-value. But we also see that our first three SNPs seem significant as well.

Note: the $-log10(p)$ notation means that we are transforming the visual of our p-value to put more emphasis on those with the lowest values (log scale). You may notice that it is $-log$ which means that on our graph we are looking for the highest value instead of the lowest value for which SNP is most significant. (think of it like $(pval)^{-5}$ now looks more like $(pval)^5$).
...
INSERT GRAPH DRAWING WITH ONLY X and Y
...
3. Confounding variable $Z$ cannot be on the causal pathway between predictor $X$ and outcome $Y$ ($X$ does not cause $Z$).

DRAWING OF ALL THREE CRITERIA
